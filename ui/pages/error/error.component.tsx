...
const ErrorPage: React.FC<ErrorPageProps> = ({ error }) => {
  ...
  useEffect(() => {
    ...
    if (sentryButtonRef.current) {
      feedback.attachTo(sentryButtonRef.current); // Attach to the ref
    }

    // close form when action button within is triggered
    return () => {
      if (feedback) {
        feedback.remove();
      }
      ...
      <section className="error-page__inner-wrapper">
        ...
        >
          ...
          {error.stack ? (
            ...
            >
              {t('errorStack')}
              <pre className="error-page__stack">{error.stack}</pre>
            </Text>
          ) : null}
        </Box>
